<template>
  <div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div class="p-4">
      <h3 class="text-xl font-semibold text-gray-800">{{ lieu.nom }}</h3>
      ...
      <div v-if="noteMoyenne > 0" class="flex items-center gap-1 mt-2">
        <div class="flex items-center">
          <span v-for="i in 5" :key="i" class="text-sm" :class="i <= noteMoyenne ? 'text-yellow-400' : 'text-gray-300'">
            ★
          </span>
        </div>
        <span class="text-xs text-gray-500 ml-1">({{ nombreAvis }})</span>
      </div>
      
      <p class="mt-2 text-gray-700 line-clamp-2">{{ lieu.description }}</p>
      
      <div class="mt-4 flex justify-between items-center">
        <div class="flex items-center gap-2">
          <span class="text-sm text-gray-500">{{ lieu.type }}</span>
          <span v-if="lieu.prix" class="text-sm font-medium text-indigo-600">{{ lieu.prix }}€</span>
          <!-- Accessibilité -->
          <span v-if="lieu.accessible_mobilite_reduite" class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full">
            ♿ PMR
          </span>
        </div>
      </div>
